...
const scored = a => !a.skipped && Number.isInteger(a.score) && a.score >= 0 && a.score <= 100;
...
export function buildReportDocument(session) {
  ...
  const articles = answers.map((a, index) => {
    ...
    return `<article><div class="answer-heading"><span>QUESTION ${index+1}</span><b>${escape(label(mode(a)))} · ${a.skipped ? 'Skipped' : scored(a) ? a.score+'/100' : 'Not scored'}</b></div>
<h2>${escape(a.question || 'Question unavailable')}</h2><h3>Reviewed answer</h3>${paragraph(a.answer || 'No reviewed transcript saved.')}
<h3>Evaluation</h3>${paragraph(a.skipped ? 'Skipped by you. Excluded from scoring and pending evaluations.' : scored(a) ? a.feedback || 'Summary feedback unavailable.' : 'Not scored. This answer is excluded from score averages.')}
